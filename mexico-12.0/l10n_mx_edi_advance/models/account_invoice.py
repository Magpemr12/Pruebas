...
class AccountInvoice(models.Model):
    ...
    def assign_outstanding_credit(self, credit_aml_id):
        ...
        for invoice in self.filtered(lambda r: r.state == 'draft'):
            ...
            taxes = credit_aml.tax_ids.compute_all(
                amount, credit_aml.currency_id or credit_aml.company_id.currency_id, 1)['taxes']
            amount += sum([tax.get('amount') for tax in taxes])
            if amount > self.amount_total:
                amount = self.amount_total
            for line in invoice.invoice_line_ids:
                factor = amount / self.amount_total if self.amount_total else 1
                adv_text = ' - CFDI por remanente de un anticipo'
                line.write({
                    'name': '%s%s' % (
                        line.name.replace(adv_text, ''), adv_text),
                    'l10n_mx_edi_total_discount': (
                        factor * line.price_unit * line.quantity) + line.l10n_mx_edi_total_discount,  # noqa
                })
