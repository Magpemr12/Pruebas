                ...
                if line.product_id:
                    ...
                    if counter == 0:
                        ...
                        if line.product_id.general_public_invoice != general_public_invoice:
                            raise UserError(_("The invoice cannot be validated and stamped because it contains \
                            one or more items of different CFDI use (donations, services, products, etc.)"))
                        if line.product_id.donation != donation:
                            raise UserError(_("The invoice cannot be validated and stamped because it contains \
                            one or more elements other than donations"))
                    counter += 1
